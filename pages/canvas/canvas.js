...
var va=29*height/667;
    ...
    ctx2.drawImage("/images/blank.png", width * 0.12 * w - 57 * w+va, width * 0.23 * h - 50 * h, (width + 20 * w)*0.8+va, (width * 0.12 * h + 100 * h + height * h * 0.22 - (width * 0.1 * h - 40 * h))); //来一张纯白色背景图
    ...
    ctx2.arc(width * 0.137 * w+va, width * 0.24 * h, 40 * w, 0, 2 * Math.PI);
    ctx2.clip();


    ctx2.drawImage(that.data.headimg, width * 0.137 * w - 40 * w+va, width * 0.27 * h - 40 * h, 80 * w, 80 * h);
    ctx2.setTransform(0.11 * w, 0, 0, 0.11 * h, 0, 0);

    ctx2.restore()

    ctx2.setFontSize(16 * w);
    ctx2.setFillStyle("black");
    ctx2.fillText(that.data.userData.name, width * 0.137 * w + 60 * w+va, width * 0.23 * h);
    ctx2.setFillStyle("#ccc");
    ctx2.fillText(that.data.userData.timeStamp, width * 0.137 * w + 50 * w+va, width * 0.29 * h);
    ctx2.setFillStyle("black");

    that.canvasTextAutoLine(str2, ctx2, width * 0.137 * w - 40 * w+va, width * 0.21* h + 60 * h, 25 * h, w);


    // if (pi > 2) {
    if(that.data.imgBox.length==1){
      ctx2.save();
      ctx2.drawImage(that.data.imgBox[0], width * 0.137 * w + va - 40 * w , width * 0.22 * h + 80 * h, width * 0.34 * w, height * h * 0.16);
      ctx2.restore();
    }else{
      for (let i = 0; i < that.data.imgBox.length; i++) {
        ctx2.save();
        ctx2.drawImage(that.data.imgBox[i], width * 0.137 * w + va - 40 * w + i * width * 0.25, width * 0.22* h + 80 * h, width * 0.24 * w, height * h * 0.14);
        ctx2.restore();
      }
    }
    ...
    ctx2.setFillStyle("#ccc");
    ctx2.fillText("地址:"+that.data.userData.address, width * 0.137 * w +va- 40 * w, width * 0.18 * h + 60 * h + height * h * 0.24);
    for (let i = 0; i < that.data.userData.others.length; i++){
      ctx2.drawImage(that.data.userData.others[i].key, width * 0.135 * w + va - 40 * w+100*w*i-20*i, width * 0.16 * h+ 60 * h + height * h * 0.27,18*w,12*h);
      ctx2.fillText(that.data.userData.others[i].value, width * 0.135 * w + va - 40 * w + 100 * w * i-20 * i+20, width * 0.160 * h + 70 * h + height * h * 0.27);
    }
